...
export const quizService = {
    ...
    try {
      ...
      const response = await together.chat.completions.create({
        messages: [
          {
            role: "system",
            content: "You are an expert quiz generator. Generate quiz questions in valid JSON format only. Do not include any explanatory text outside the JSON."
          },
          {
            role: "user",
            content: prompt
  ...
  createQuizPrompt(config: QuizConfig): string {
    ...
    return `Generate ${config.questionCount} multiple choice questions about ${config.topic} at ${difficultyInstructions[config.difficulty]} difficulty.
...
Note: correctAnswer should be the index (0-3) of the correct option in the options array.`;
  ...
  async generateMockQuiz(config: QuizConfig): Promise<Quiz> {
    ...
    const topicQuestions = this.getTopicSpecificQuestions(config.topic, config.questionCount);
      ...
      'Operating Systems': [
        {
          ...
          options: ['Compile programs', 'Manage hardware resources', 'Create databases', 'Design user interfaces'],
          correctAnswer: 1,
          explanation: 'The primary purpose of an OS is to manage hardware resources and provide services to applications.'
        },
        {
          id: 'os2',
          question: 'Which scheduling algorithm gives priority to the process with the shortest execution time?',
          options: ['FCFS', 'SJF', 'Round Robin', 'Priority Scheduling'],
          correctAnswer: 1,
          explanation: 'Shortest Job First (SJF) algorithm prioritizes processes with the shortest execution time.'
        },
        {
          id: 'os3',
          question: 'What is a deadlock in operating systems?',
          options: ['A crashed program', 'A circular wait condition', 'A memory leak', 'A network error'],
          correctAnswer: 1,
          explanation: 'Deadlock occurs when processes are waiting for each other in a circular chain.'
        }
      ],
      'Data Structures': [
        {
      ...
      'Database Management Systems': [
        {
          ...
          options: ['Atomicity, Consistency, Isolation, Durability', 'Access, Control, Integration, Data', 'Automatic, Concurrent, Independent, Distributed', 'Authentication, Compression, Indexing, Distribution'],
          correctAnswer: 0,
      ...
      'Object-Oriented Programming': [
        {
          ...
          options: ['Creating multiple objects', 'Hiding internal implementation details', 'Inheriting from parent class', 'Overriding methods'],
          correctAnswer: 1,
          explanation: 'Encapsulation is the bundling of data and methods while hiding internal implementation details.'
        }
      ]
    };
  ...
  async uploadPDFQuiz(file: File): Promise<Quiz> {
    ...
    const mockQuestions: Question[] = Array.from({ length: 10 }, (_, i) => ({
      ...
      correctAnswer: Math.floor(Math.random() * 4),
      explanation: `Explanation extracted from PDF content for question ${i + 1}.`,
    }));
